# stacks/azure/mattermost-vm-crossguard-blob/network.tf
#
...
#
# Production note: these two VMs represent two federated Mattermost instances.
# In a real deployment they'd live in separate VNets (or even separate subscriptions)
# with VNet peering or a shared storage private endpoint for blob access.

resource "azurerm_virtual_network" "crossguard_blob" {
  depends_on = [data.azurerm_resource_group.crossguard_blob]
...
resource "azurerm_subnet" "vm_a" {
  ...
  resource_group_name  = var.resource_group_name
  virtual_network_name = azurerm_virtual_network.crossguard_blob.name
