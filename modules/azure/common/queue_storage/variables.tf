...
variable "inbound_queue_name" {
  description = "Azure Queue name for messages arriving into this Mattermost instance (external → this instance)"
  type        = string
  default     = "crossguard-inbound"
}

variable "outbound_queue_name" {
  description = "Azure Queue name for messages leaving this Mattermost instance (this instance → external)"
  type        = string
  default     = "crossguard-outbound"
}

variable "blob_container_name" {
  description = "Blob container for CrossGuard file transfers. Required when file_transfer_enabled is true on any connection."
  type        = string
...
variable "public_network_access_enabled" {
  description = <<-EOT
    ...
    Production: set to false and deploy a private endpoint so the VM accesses
    Queue/Blob Storage over the VNet without traversing the public internet.
  EOT
  type    = bool
  default = true
}
...
variable "allowed_subnet_ids" {
  description = <<-EOT
    ...
    Production: populate with the VM subnet ID and set public_network_access_enabled = false.
  EOT
  type    = list(string)
  default = []
}
