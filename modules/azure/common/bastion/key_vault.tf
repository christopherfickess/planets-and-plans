# modules/azure/common/bastion/key_vault.tf
...
resource "azurerm_role_assignment" "kv_pde_secrets_user" {
  scope                = azurerm_key_vault.jumpbox.id
  role_definition_name = "Key Vault Secrets User"
  principal_id         = var.azure_pde_group_object_id
}

resource "azurerm_key_vault_secret" "jumpbox_private_key" {
  depends_on   = [azurerm_role_assignment.kv_secrets_officer]
  name         = "${var.unique_name_prefix}-${var.keyvault_private_key_secret_name}"
...
resource "azurerm_key_vault_secret" "jumpbox_public_key" {
  depends_on   = [azurerm_role_assignment.kv_secrets_officer]
  name         = "${var.unique_name_prefix}-${var.keyvault_public_key_secret_name}"
